...
## Where this lives in IRIS

- `src/rex3.rs` — registers, the GFIFO, command dispatch, DCB devices (VC2, XMAP9,
  CMAP, BT445) and the display refresh thread.
- `src/rex3_generic.rs` — the one generic draw pipeline (blocks, spans, lines,
  scr2scr, host read/write, shading, patterns, clipping, CID).
- `src/rex3_shape.rs` — decodes DrawMode0/DrawMode1 and the clip state into the
  compile-time "shape" a specialised draw function is built for.
- `src/rex3_shaders.rs` — 400+ precompiled specialisations of the generic
  pipeline, generated by `tools/gen_rex3_shaders.py` from a profile of real draw
  modes. Do not edit by hand.
- `src/rex3_jit/` — the optional Cranelift JIT (`--features rex-jit`) for modes
  outside that set; `src/rex3_profile.rs` persists the modes seen to
  `~/.iris/rex-jit-profile.bin`.
- `src/rex3_tests.rs` — interpreter/JIT equivalence and drawing tests.
