...
- **Compiled-function ABI = handler ABI**: `fn(decoded, state) -> continuation`. **Exit model (`[Q6.2]` closed): return to the main loop** — the epilogue drains state, writes PC + the **transfer flag** (a `jr` exit that skips the flag is never discovered), and returns; the loop's int-check-before-dispatch at the landing instruction is exactly the §3.2 boundary semantics. **As-built correction:** `step_jit` (mips_exec.rs) does *not* run `step_preamble!` — it was moved off the compiled-code hit path deliberately — and both compile paths pass `skip_entry_preamble = true`, so that landing-instruction check does not exist. What holds instead: an externally-dispatched entry word runs unchecked, deferring delivery by **at most one entry dispatch**, because every non-entry word and every *internal back-edge* onto an entry word still pays its own preamble (codegen's `entry_body_blocks` split). Pinned by `skip_entry_preamble_true_bypasses_the_entry_words_own_checks` and `..._still_checks_on_an_internal_back_edge_into_entry`. Exceptions were already delivered inside via shared `handle_exception` (§3.3); the gateway adds nothing on exit.
