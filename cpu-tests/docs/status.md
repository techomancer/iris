...
| `fpu/vec_arith_single`, `fpu/vec_arith_double`, `fpu/vec_sqrt`, `fpu/cvt_s_d_rounds` | **new** — Inexact is never computed for arithmetic; see below |
| `fpu/cvt_out_of_range` | **new** — Invalid not raised for an out-of-range finite conversion; see below |
...
### The `fpu/vec_*` and `fpu/cvt_*` failures: a deliberate design trade

Not the exception-model findings (6-10), which are about *when a trap is taken
and what it writes*. These are about **which flag bits a correctly-computed
result should have raised** — in every one of the five, the numeric result is
right and only `o.flags`/`o.cause` is wrong:

```
fpu/vec_sqrt      o.flags [sqrt.s]  got 0x0  want 0x1      (missing Inexact)
fpu/vec_arith_*   o.flags [add.s=6] got 0x0  want 0x5      (missing Inexact)
fpu/cvt_s_d_rounds  fcsr_flags()    got 0x4  want 0x5      (has Overflow, missing Inexact)
fpu/cvt_out_of_range  o.cause & FP_V got 0x0 want 0x10     (missing Invalid)
```
...
compute for ordinary arithmetic, and nothing in IRIX depends on them").

So this is a **known gap, not a regression**, even though these tests were
passing at the 2026-08-19 measurement. Full writeup:
`rules/jitv2/fpu-flags-are-computed-not-read.md`. Read it before attempting a
fix — setting Inexact means computing it in *both* engines, including in emitted
IR, which is exactly the cost the current design declined.
